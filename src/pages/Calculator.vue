...
<section class="flex w-full ">
    <div class="m-auto ">
        ...
        <p class="text-3xl text-right mt-10 w-32 h-10 overflow-x-scroll" style="direction:rtl">{{currNum}}</p>
        <div class="h-10">
            <small v-if="selectedOperation">{{prevNum}} {{selectedOperation}} {{currNum}}</small>
        </div>
        <div class="grid grid-cols-4 gap-1 ">
            <button @click="pressed(i)" class="p-2 border shadow rounded w-10 h-10" v-for="i in num" :key="i">{{i}}</button>
...
export default {
    setup() {
        const num = ref([1, 2, 3, '+', 4, 5, 6, '-', 7, 8, 9, '*', 'C', 0, '=', '/'])
        const operations = ['+', '-', '*', '/']
